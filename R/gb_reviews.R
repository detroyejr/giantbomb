#' GiantBomb Reviews
#'
#' Retrieve a review or reviews from the GiantBomb API.
#'
#' @md
...
#' \dontrun{
#' # Get 10 companies.
#' reviews <- gb_reviews(n = 10)
#'
#' # Get a single company with an id.
#' reviews <- gb_reviews(id = 360)
#' }
#'
...
#'
#' @export
gb_reviews <- function(id = NULL, n = 100, field_list = NULL, limit = 100,
                       offset = 0, sort = NULL, filter = NULL,
                       user_agent = NULL, delay = 30, api_key = gb_key()) {
  if (!is.null(id)) {
    stopifnot(length(id) == 1)
    url <- paste0(base, "review/", id, "/")
  } else {
    url <- paste0(base, "reviews/")
  }
