#' GiantBomb Games
#'
#' Retrieve a game or games from the GiantBomb API.
#'
#' @md
...
#' games <- gb_games(filter = "name:bioshock", sort = "date_added:asc")
#' }
#'
...
#'
#' @export
gb_games <- function(id = NULL, n = 100, field_list = NULL, limit = 100,
                     offset = 0, sort = NULL, filter = NULL,
                     user_agent = NULL, delay = 30, api_key = gb_key()) {
  if (!is.null(id)) {
    stopifnot(length(id) == 1)
    url <- paste0(base, "game/", id, "/")
  } else {
    url <- paste0(base, "games/")
  }
