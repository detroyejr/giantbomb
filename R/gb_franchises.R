#' GiantBomb Franchises
#'
#' Retrieve a franchise or franchises from the GiantBomb API.
#'
#' @md
...
#' \dontrun{
#' # Get 10 companies.
#' franchises <- gb_franchises(n = 10)
#'
#' # Get a single company with an id.
#' franchises <- gb_franchises(id = 360)
#' }
#'
...
#'
#' @export
gb_franchises <- function(id = NULL, n = 100, field_list = NULL, limit = 100,
                          offset = 0, sort = NULL, filter = NULL,
                          user_agent = NULL, delay = 30, api_key = gb_key()) {
  if (!is.null(id)) {
    stopifnot(length(id) == 1)
    url <- paste0(base, "franchise/", id, "/")
  } else {
    url <- paste0(base, "franchises/")
  }
